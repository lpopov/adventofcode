...
class Day02_2 {
	...
	public function run() {
		foreach($this->inputArr as $row) {
			$valuesArr = explode("\t", $row);
			
			$result = null;
			
			for( $i = 0; $i < sizeof($valuesArr); $i++) {
				for( $j = $i + 1; $j < sizeof($valuesArr); $j++){
					if($valuesArr[$i] > $valuesArr[$j]) {
						if($valuesArr[$i] % $valuesArr[$j] === 0) {
							$result = $valuesArr[$i] / $valuesArr[$j];
							break;
						}
					} else {
						if($valuesArr[$j] % $valuesArr[$i] === 0) {
							$result = $valuesArr[$j] / $valuesArr[$i];
							break;
						}
					}
				}
...
foreach($inputs as $input) {
	$day = new Day02_2($input);
	$day->run();
	echo "The checksum is: " . $day->getChecksum() . "\n";
}
